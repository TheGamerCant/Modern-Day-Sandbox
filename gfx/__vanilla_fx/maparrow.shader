...
ConstantBuffer( 3, 32 ) # For arrow shader
{
	...
	float TotalLength;          // arrow's full length in map units
	float UvToAbsoluteLength;   // multiplier that turns uv_isHead_variant.x into a map-unit position
};
...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT_MAPARROW Input ) : PDX_COLOR
		{
			...
			// fade_in / fade_out are in map units along the curve. Recover the
			// absolute position by undoing the ANIM_TEXTURE time offset and
			// scaling. The +EPS / max(fade, EPS) trick avoids a 0/0 NaN at the
			// endpoints when fade is 0, so we don't need ternaries.
			const float Epsilon = 0.0001f;
			float AlongUV = Input.uv_isHead_variant.x + vTime_IsSelected_FadeInOut.x * vOfsX_vOfsTex_vOfsFx.y;
			float AbsLen = AlongUV * UvToAbsoluteLength;
			vMaskValue *= saturate( ( AbsLen + Epsilon ) / max( vTime_IsSelected_FadeInOut.z, Epsilon ) );
			vMaskValue *= saturate( ( TotalLength - AbsLen + Epsilon ) / max( vTime_IsSelected_FadeInOut.w, Epsilon ) );
