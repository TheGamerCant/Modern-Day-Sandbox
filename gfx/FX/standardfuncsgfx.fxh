...
PixelShader = 
{
	...
	void gradient_border_apply_culture_map( inout float3 vColor, float3 vNormal, float2 vUV, 
		in sampler2D TexCh1, in sampler2D TexCh2, 
		float vOutlineMult, float2 vOutlineCutoff, float2 vCamDistOverride, inout float vBloomAlpha )
	{
		...
		float3 primaryCultureRgb, secondaryCultureRgb;
		
		float vAlpha1 = gradient_border_process_channel( primaryCultureRgb, vColor, vGBCamDistCh1, vNormal, vUV2, TexCh1, TexCh2, vOutlineMult, vOutlineCutoff.x, GB_STRENGTH_CH1 );
		float vAlpha2 = gradient_border_process_channel( secondaryCultureRgb, vColor, vGBCamDistCh1, vNormal, vUV, TexCh1, TexCh2, vOutlineMult, vOutlineCutoff.x, GB_STRENGTH_CH1 );
		
		float3 outRgb = primaryCultureRgb;
		
		if (primaryCultureRgb.r != secondaryCultureRgb.r || primaryCultureRgb.g != secondaryCultureRgb.g || primaryCultureRgb.b != secondaryCultureRgb.b) {
			float r_x = vUV.x * MAP_SIZE_X;
			float r_y = vUV.y * MAP_SIZE_Y;
			
			float pixels_until_new_diagonal = 12.0f;
			float line_width = 4.0f;

			float pattern = fmod(r_x + r_y, pixels_until_new_diagonal);
			
			outRgb = lerp(primaryCultureRgb, secondaryCultureRgb, step(pattern, line_width));
		}
		
		float TranspA = tex2D( TexCh2, vUV ).g;		
		vColor = lerp( vColor, outRgb, ( GB_OPACITY_NEAR + ( 1.0f - vGBCamDist ) * ( GB_OPACITY_FAR - GB_OPACITY_NEAR ) ) * TranspA );
