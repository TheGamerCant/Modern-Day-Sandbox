...
void replaceEntriesFunction(const std::filesystem::path& modDirectory, const std::string& fileName){
	...
	std::ifstream mapTypesToReplaceFile(fileName);
...
//	}
	...
	std::ifstream buildingsToRepalceFile(modFilePath);
	if (!buildingsToRepalceFile) {
	...
	std::filesystem::rename(tempFilePath, modDirectory);
}
